...
if __name__ == "__main__":  
    ...
    if len(sys.argv) == 2:
        cmd = "%s %s@%s %s/qstat -u%s" % (remoteLoginCmd, userName, hostName, utilityPath, userName)
    else:
        cmd = "%s %s@%s %s/qstat %s" % (remoteLoginCmd, userName, hostName, utilityPath, string.join(sys.argv[2:]))
